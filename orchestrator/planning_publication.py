...
async def verify_publication(store, github, row, pr_number, *, allow_descendant=False):
    """Observe publication evidence and activate only the exact reserved SHA."""
    repo=row['repository']
    ...
    # Only the exact reserved planning commit can enter prototype execution.
    # A legitimate descendant may resolve association after a crash, but its
    # extra commits have no planning handoff and therefore remain un-routed.
    if head.get("sha") == intent["head_sha"]:
        ...
        if before != after:
            ...
            store.record_recovery_audit("planning:" + row["publication_id"], dict(
                ...
                status="applied",
                action_type="project",
                reason="verified exact planning publication activated through shared controller",
